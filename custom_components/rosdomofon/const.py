...
# Детектор лиц работает на СТОРОНЕ сервиса DeepFace (не в Home Assistant).
# opencv (Haar-каскад) — самый совместимый (есть в любом образе, не качает
# моделей), но самый слабый: теряет лица под углом/в темноте и грубо выравнивает
# лицо перед вычислением эмбеддинга. Поэтому это безопасный дефолт «чтобы
# завелось везде». Для реального качества в настройках лучше выбрать yunet
# (точный и лёгкий) или mtcnn/retinaface — они точнее, но тяжелее по CPU.
